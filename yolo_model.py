...
device = 'cuda' if torch.cuda.is_available() else 'cpu'
model = YOLO("best_yolo11s.pt").to(device)
...
def process_frame(frame):
    ...
    results = model.predict(rgb_frame, verbose=True, conf=0.25, device=device, classes=classes_to_detect)

    # Если есть результаты обнаружения
